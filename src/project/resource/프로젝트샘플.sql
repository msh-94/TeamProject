...
create table company(                           # 4.회사부가정보(구독자사이트 = 본사 하위사이트)Table
    ...
    logno int unsigned unique not null, 		 -- 로그번호(fk)    
    cName varchar(30) not null unique , 		 -- 사이트명
    area varchar(10) not null, 					 -- 서비스지역
    service varchar(50),  						 -- 서비스내용
    constraint foreign key(logno) references log(logno)    
);
...
# 4. 샘플데이터(company Table): 본사 구독자사이트(=본사 하위사이트) 부가정보
insert into company (logno,cName,area,service) values
( 1 , '콜마이택시', '부산', '신규회원 5000point  + 택시요금 3% 적립'),
( 2 , '콜마이택시2', '부산', '신규회원 5000point  + 택시요금 3% 적립'),
( 3 , '스마트카택시', '대구', '신규회원 4000point + 택시요금 4% 적립'),
